    ...
    if (!this.props.user.profile_pic || this.props.user.profile_pic == "") {
      ...
      return(
        <View style={{width: dimensions.width, flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>
          <View style={[userStyles.pic, userStyles.initials, {width: this.props.width, height: this.props.height, borderColor: (this.props.user.phone) ? colors.gradientGreen : colors.accent, borderRadius: this.props.width / 2}]}>
            <Text style={{fontFamily: 'Roboto', fontSize: 18, color: (this.props.user.phone) ? colors.gradientGreen : colors.accent}}>{ initials }</Text>
          </View>
        </View>
      )
    } else {
      return <Image style={{borderWidth: 1, borderColor: (this.props.user.phone && !this.props.accent) ? colors.gradientGreen : colors.accent, width: this.props.width, height: this.props.height, borderRadius: this.props.height / 2}} source={{uri: this.props.user.profile_pic}} />
    }
  }
}
